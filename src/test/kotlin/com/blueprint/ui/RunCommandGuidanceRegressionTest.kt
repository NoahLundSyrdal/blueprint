...
class RunCommandGuidanceRegressionTest {
    ...
    fun `manual run guidance names likely entry files and fallback search targets`() {
        ...
        assertTrue(source.contains("val strongestCandidate = candidates.firstOrNull()"))
        assertTrue(source.contains("val otherCandidates = candidates.drop(1)"))
        assertTrue(source.contains("Strongest candidate right now: ${'$'}it."))
        assertTrue(source.contains("Other strong candidates: ${'$'}{otherCandidates.joinToString(\", \")}."))
