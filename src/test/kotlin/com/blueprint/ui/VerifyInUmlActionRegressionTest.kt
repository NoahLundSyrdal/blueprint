...
class VerifyInUmlActionRegressionTest {
    ...
    fun `post apply flow exposes Verify In UML action and reread copy`() {
        ...
        assertTrue(source.contains("Blueprint already refreshed the code-backed UML from disk after apply. Refresh UML From Code to verify again whenever you want to rerun that reread."))
        assertTrue(source.contains("appendLine(\"- Automatic refresh after apply already reread the changed code from disk.\")"))
        assertTrue(source.contains("Refresh UML From Code reruns that code reread when you want an explicit verification click."))
