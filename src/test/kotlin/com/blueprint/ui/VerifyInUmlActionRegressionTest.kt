...
class VerifyInUmlActionRegressionTest {
    ...
    fun `post apply flow exposes Verify In UML action and reread copy`() {
        ...
        assertTrue(source.contains("Verify In UML rereads the changed code from disk, or use Refresh UML From Code to verify again later."))
        assertTrue(source.contains("appendLine(\"- Verify In UML rereads the changed code from disk.\")"))
        assertTrue(source.contains("Verify In UML rereads the changed code from disk. Refresh UML From Code stays available anytime after more edits."))
        assertTrue(source.contains("Verify In UML rereads the changed code from disk. Open Changed Files to inspect what Blueprint wrote before you rerun the app."))
