...
class EmptyStatePartialRefreshRegressionTest {
    ...
    fun `partial refresh note references actual skipped counts reasons and example paths`() {
        ...
        assertTrue(source.contains("val examplePaths = context.skippedFiles.take(2).joinToString(\", \") { it.path }"))
        assertTrue(source.contains("\${context.skippedFiles.size} Python path\${if (context.skippedFiles.size == 1) \" was\" else \"s were\"}"))
    }
}
