...
class ApplySuccessCopyRegressionTest {
    ...
    fun `apply success message keeps verification steps clear and compile safe`() {
        ...
        assertTrue(source.contains("val changedPathCountLine = if (changedPaths.isEmpty()) {"))
        assertTrue(source.contains("\"Changed paths: none.\""))
        assertTrue(source.contains("\"Changed paths (\${changedPaths.size}): \${changedPaths.joinToString(\", \")}\""))
