...
class RunCommandConfidenceRegressionTest {
    ...
    fun `post run summary explains why the current run command remains the default`() {
        ...
        assertTrue(source.contains("\"Run command confidence: Blueprint verified \$runCommand and still sees other likely entry files (\${candidates.joinToString(\", \")}), so treat this as the current best default and re-check those files if you switch app targets.\""))
