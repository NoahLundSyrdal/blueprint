...
class EmptyStateGuidanceRegressionTest {
    ...
    fun `empty uml guide distinguishes supported python projects from sparse folders`() {
        ...
        assertTrue(source.contains("Partial refresh note: \${context.skippedFiles.size} Python path\${if (context.skippedFiles.size == 1) \" was\" else \"s were\"} skipped during Refresh UML From Code (\$topReasons)."))
        assertTrue(source.contains("The current code-backed UML still reflects the Python files Blueprint could read, so inspect skipped paths like \$examplePaths before Generate Code Diff if the UML looks incomplete, or Refresh UML From Code to verify after you adjust the folder or files."))
