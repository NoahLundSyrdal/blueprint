...
class EmptyStateProductPromiseRegressionTest {
    ...
    fun `python empty state explains the full blueprint loop`() {
        assertTrue(source.contains("Blueprint can open any Python folder, draw a code-backed UML diagram, help you refine it with chat or direct edits, generate a reviewed code diff, apply approved changes, refresh UML from code to verify, and run the changed app."))
    ...
    fun `non python empty state explains the full blueprint loop once python files exist`() {
        assertTrue(source.contains("When this folder has Python files, Blueprint can turn them into a code-backed UML diagram, help you refine that UML, generate a reviewed code diff, apply approved changes, refresh UML from code to verify, and run the changed app."))
