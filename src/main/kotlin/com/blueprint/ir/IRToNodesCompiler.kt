...
class IRToNodesCompiler(private val project: Project) {
    ...
    fun compile(ir: ArchitectureIR): CompileResult {
        if (ir.components.isEmpty()) {
            return CompileResult(emptyList(), listOf("IR has no components; nothing to compile."))
        }
        ...
        // Stable ids per component.
        val nodeIdsByComponent: Map<String, String> = ir.components.associate { c -> c.id to stableNodeId(c.id) }

        // Order components so schemas/models come first, services/adapters next,
        // cli/test/docs last — matches downstream execution ordering.
        val orderedComponents = ir.components.sortedWith(
            compareBy<Component> { kindOrder(it.kind) }.thenBy { it.ownership.files.firstOrNull() ?: "" }.thenBy { it.name }
        )
        ...
        return CompileResult(nodes, warnings)
    }

    private fun buildTitle(index: Int, component: Component): String {
        val prefix = index.toString().padStart(2, '0')
        val kindLabel = component.kind.label()
        return "$prefix ${component.name} $kindLabel"
    }
    ...
    private fun buildOutputs(component: Component, contractsById: Map<String, Contract>): List<NodeContract> {
        ...
        // No explicit contract — synthesize one from the component shape.
    ...
    private fun buildInputs(
        ...
        val fromContracts = component.requires.mapNotNull { id ->
            // requires may name a contract id directly, or a component whose
            // first provided contract we adopt.
    ...
    private fun resolveDependencies(
        ...
        // From `requires`: resolve contract id → providing component, or direct
        // component id / name.
        ...
        // From edges: CALLS / EXTENDS / CONTAINS / REFERENCES / REGISTERS_WITH.
        ir.edges
    ...
    private fun buildDescription(component: Component, ir: ArchitectureIR): String = buildString {
        ...
        if (component.provides.isNotEmpty()) {
            appendLine("Provides: ${component.provides.joinToString(", ")}")
        }
        if (component.requires.isNotEmpty()) {
            appendLine("Requires: ${component.requires.joinToString(", ")}")
        }
        if (component.concurrency == Concurrency.ASYNC) {
            appendLine("Concurrency: async")
        }
        if (component.opaqueAnnotations.isNotEmpty()) {
            appendLine()
            appendLine("Opaque annotations preserved:")
            component.opaqueAnnotations.forEach { ann ->
                appendLine("- ${ann.symbol} (${ann.reason})")
            }
        }
