 ...
 *
 * Each parsed entity becomes a MODEL component. On top of that, v1 emits
 * a small set of scaffolding components — service, CLI, test, docs — all
 * owned under a package slug derived from the primary entity. Relationships
 * become IR edges.
 *
 * The transformation is pure; see [UmlToIR]. The service class is a thin
 * wrapper so the DI lifecycle is still Project-scoped.
 */
@Service(Service.Level.PROJECT)
class UmlIRImporter(private val project: Project) {
    fun toIR(parsed: UmlImportService.ParsedUml): ArchitectureIR =
        UmlToIR.toIR(parsed, project.name)
}

/** Pure projection of parsed UML → ArchitectureIR. */
object UmlToIR {

    fun toIR(parsed: UmlImportService.ParsedUml, projectName: String): ArchitectureIR {
        if (parsed.entities.isEmpty()) {
            return ArchitectureIR(project = ProjectMeta(name = projectName))
        }
